        ...
        <div class="col-lg-6">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route("guzbyte.admin.ticket.agent.update", [$agent->id]) }}" method="POST">
                        @csrf
                        @method("PATCH")
                        <div class="form-group">
                            <input type="email" name="email" class="form-control @error('email')
                                in_invalid @enderror" placeholder="Agent Email" value="{{ $email }}">
                            @error("email")
                           ...
                           <select name="category" id="" class="form-control @error("category") is-invalid @enderror">
                            <option value="">Select ticket category</option>
                                @foreach ($categories as $category)
                                    ...
                                    <option value="{{ $category->id }}"  @if ($category->id == $agent->category)
                                        selected
