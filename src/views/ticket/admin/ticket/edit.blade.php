    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route("guzbyte.admin.ticket.update", [$ticket->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method("PATCH")
                    ...
                    <div class="form-group">
                        <label for="agents">Re-assign Agent</label>
                        <select name="agent" id="agents" class="form-control">
                            <option value="">Select agent</option>
                            @foreach ($agents as $agent)
                                <option value="{{ $agent->id }}" @if ($agent->id === $ticket->agent_id)
                                    selected
                                @endif >{{ App\User::find($agent->user_id)->name }}</option>
                            @endforeach
                        </select>
                    </div>
